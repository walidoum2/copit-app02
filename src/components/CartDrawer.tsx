      ...
      <div className={`drawer ${show ? "show" : ""}`}>
        ...
        <div className="drawer-body">
          {items.length === 0 ? (
            <div className="empty-state">
              ...
              <p style={{ marginTop: 12, fontSize: 14 }}>{t("cart_empty")}</p>
            </div>
          ) : (
            items.map((item) => (
              <div key={item.variantId} className="cart-line">
                <div className="cart-thumb">
                  {item.image ? (
                    ...
                    <svg viewBox="0 0 200 130" width="50%" fill="none" stroke="currentColor" strokeWidth="1.2">
                ...
                <div className="cart-info">
                  ...
                  <div className="qty-row">
                    <button onClick={() => changeQty(item.variantId, -1)} aria-label="Decrease quantity">−</button>
                    <span>{item.quantity}</span>
                    <button onClick={() => changeQty(item.variantId, 1)} aria-label="Increase quantity">+</button>
                  </div>
        ...
        {items.length > 0 && (
          <div className="drawer-foot">
            <div className="sum-row" style={{ justifyContent: "space-between", display: "flex", fontSize: 14, marginBottom: 8 }}>
              <span>{t("subtotal")}</span>
              <span className="mono" style={{ fontWeight: 700 }}>{money(subtotal)}</span>
            </div>
            <button
